...
main() {
    ...
    if [ -z "$LATEST" ]; then
        INSTALL_URL="git+https://github.com/${REPO}.git#egg=budget"
        printf 'no release — installing from main\n'
    else
        INSTALL_URL="git+https://github.com/${REPO}.git@${LATEST}#egg=budget"
        printf 'installing budget %s…\n' "$LATEST"
    fi
    "$PYTHON" -m pip install --quiet "$INSTALL_URL"
    "$PYTHON" -m budget --install
    printf '\n\033[32m✓\033[0m  budget installed — restart Claude Code\n'
    printf '\n    Cost control: hard token/cost limits prevent runaway agent spend\n\n'
}
