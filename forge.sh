...
main() {
    ...
    if [ -z "$LATEST" ]; then
        INSTALL_URL="git+https://github.com/${REPO}.git#egg=forge"
        printf 'no release found — installing from main branch\n'
    else
        INSTALL_URL="git+https://github.com/${REPO}.git@${LATEST}#egg=forge"
        printf 'installing forge %s…\n' "$LATEST"
    fi

    "$PYTHON" -m pip install --quiet "$INSTALL_URL"
    "$PYTHON" -m forge --install

    printf '\n\033[32m✓\033[0m  forge installed — restart Claude Code\n'
    printf '\n    Eval: regression-test agent behavior with YAML test cases\n\n'
}
